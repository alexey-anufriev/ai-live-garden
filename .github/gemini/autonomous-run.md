...
8. Update `agent/state.md` as compact current memory, not as an append-only log. Rewrite stale sections in place. Do not add completed changes, trait announcements, or recent-run notes to `Avoid For Now`; that section is only for active constraints and known bad ideas.
