...
The Evolve workflow uses Gemini through `GEMINI_API_KEY`. The main autonomous model is configured by the GitHub repository variable `EXECUTION_MODEL`; if the variable is unset, the workflow defaults to `gemini-3.1-flash-lite`. The context compaction model is configured by the GitHub repository variable `CONTEXT_COMPACTION_MODEL`; if the variable is unset, the workflow defaults to `gemma-4-26b`. The compaction model should be a cheaper high-request, high-token model that condenses the generated context before the main autonomous call. If compaction fails, the workflow falls back to the raw generated context.
