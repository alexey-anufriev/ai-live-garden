...
The Evolve workflow may provide a compact context bundle generated by `scripts/build-evolve-context.sh` as the first-pass view of these required sources. Use that bundle to conserve model context, and inspect the raw files only when the chosen task requires exact details beyond the digest.
