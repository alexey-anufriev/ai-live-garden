...
The Agent workflow may provide a compact context bundle generated by `scripts/build-agent-context.sh` as the first-pass view of these required sources. Use that bundle to conserve model context, and inspect the raw files only when the chosen task requires exact details beyond the digest. The workflow may optionally pass that bundle through a cheaper context-compaction model before the main autonomous Gemini call. Compaction is only a quota-saving digest of continuity material; exact repository rules, templates, current garden-state digest, and validation scripts remain authoritative.
...
The chronicle is maintained by the Story workflow, not by the normal Agent workflow.

The Agent workflow agent may read the chronicle for continuity, but must not modify files under `story/`.
...
The Agent workflow uses Gemini through `GEMINI_API_KEY`. The main autonomous model is configured by the GitHub repository variable `EXECUTION_MODEL`; if the variable is unset, the workflow defaults to `gemini-3.1-flash-lite`. The context compaction model is configured by the GitHub repository variable `CONTEXT_COMPACTION_MODEL`; if the variable is unset, the workflow defaults to `gemma-4-26b-a4b-it`. The compaction model should be a cheaper high-request, high-token model that condenses the generated context before the main autonomous call. If `CONTEXT_COMPACTION_MODEL` is non-empty and compaction fails, the workflow must fail before the main autonomous step instead of silently falling back to the raw generated context. Setting `CONTEXT_COMPACTION_MODEL` to an empty value disables compaction and uses the raw generated context directly.

If the main Gemini autonomous step fails because of API quota exhaustion, billing limits, authentication, or provider availability, the run must fail without committing. This is an environment capacity issue, not a garden evolution task. Do not work around it by committing partial changes, removing validation, suppressing errors, or adding unbounded retries. The appropriate actions are to wait for quota reset, reduce run frequency, change `EXECUTION_MODEL`, use `CONTEXT_COMPACTION_MODEL`, or ask the human to adjust quota, billing, or credentials.

If tests fail after the main Gemini autonomous step, the Agent workflow may ask Gemini to repair the test failure and retry validation, using the same bounded repair action as the format checks. Repairs must preserve the autonomous run's required journal, summary, state, and README updates. If `mvn -B test` still fails after the final repair attempt, the workflow must fail before advancing, archiving, or committing.
...
The Agent workflow enforces active summary retention by running `scripts/archive-summaries.sh`. That script keeps the newest active summary files directly under each cadence folder and moves older files unchanged into that cadence's archive: 100 daily summaries, 50 weekly summaries, 12 monthly summaries, and 10 yearly summaries.
