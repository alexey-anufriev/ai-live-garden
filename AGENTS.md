...
The Evolve workflow may provide a compact context bundle generated by `scripts/build-evolve-context.sh` as the first-pass view of these required sources. Use that bundle to conserve model context, and inspect the raw files only when the chosen task requires exact details beyond the digest. The workflow may optionally pass that bundle through a cheaper context-compaction model before the main autonomous Gemini call. Compaction is only a quota-saving digest of continuity material; exact repository rules, templates, current garden-state digest, and validation scripts remain authoritative.
