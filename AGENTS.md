...
- `agent/plans/latest.md` is the latest Project Manager direction. When it exists and baseline repair is not required, it is the highest product priority for autonomous runs.
...
3. If baseline tests or worktree policy are failing, make repair the focused task. Otherwise, if `agent/plans/latest.md` exists, choose exactly one listed PM direction and set `pmDirection` in `.agent-run.json` to its label.
