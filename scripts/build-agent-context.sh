...

{
  ...
  echo "- For an evolution run, use the Baseline Shadow Simulation section as the acceptance baseline and declare a truthful evaluation. Run the exact candidate preflight when possible; otherwise leave preflight false/null. The harness measures and writes the authoritative delta before acceptance. Unit tests alone are not ecological validation."
