...
append_baseline_shadow_result() {
  ...
  echo "- The target classifies impact; it is not a merge gate. Safe coherent code is committed with a \`target-met\`, \`partial-progress\`, \`inert\`, \`measurement-saturated\`, or \`wrong-direction\` verdict for the next run. A \`measurement-saturated\` verdict means every final environmental target value landed on the same 0/100 boundary; it does not prove the mechanism was inactive. Invalid, unmeasured, test-failing, policy-violating, or unsafe candidates remain off main and may be preserved on an \`agent-rejected/\` branch."
...
append_shadow_feedback() {
  ...
  if [[ -f "$shadow_feedback_file" ]]; then
    echo "A previous experiment verdict or invalid-candidate report is supplied in full below. This is mandatory decision input, not optional history. If it is a verdict, the evaluated code is already on main; inspect it and decide explicitly to reuse, revise, or abandon it. If a preserved branch is listed, inspect that exact commit. The verdict's Experiment Lineage contains only the current experiment and one immediate predecessor. Its continuity value is advisory: \`matched\` means a reuse/revision touched a predecessor path, \`diverged\` means it did not, \`abandoned\` means the agent explicitly changed causal direction, and \`unavailable\` means no predecessor paths were recorded. When you choose \`reuse\` or \`revise\`, normally change the predecessor's listed paths or explain why a different path is required; when you change causal direction, choose \`abandon\` explicitly. Set \`causalReach.feedbackReference\` to a concise reference to the specific predecessor mechanism or path being reused/revised, or to the mechanism and evidence being abandoned. An exact reference produces stronger lineage; if it is omitted, the harness records unavailable continuity instead of rejecting otherwise substantive code. An escalation of \`diagnose-or-abandon\` means two related experiments on the same target stalled; before another coefficient change, inspect the actual phase/gate and either make one focused diagnostic change or explicitly abandon the mechanism. This is guidance, not a merge gate. State the decision and evidence in \`evidence.bottleneck\` or \`observations\`. Inert or wrong-direction code should normally be corrected or reverted before adding a disconnected mechanism. For \`measurement-saturated\`, do not claim the mechanism was inactive from the final delta alone and do not reverse the goal merely to obtain a nonzero result; inspect the existing flow and choose a concrete revision or abandonment."
    echo
    awk '
      /^## Prior Feedback$/ {
        prior_sections++
