...
if (( observed_ticks < required_ticks )); then
  set_ready false
  echo "Observation window is still active: ${observed_ticks}/${required_ticks} AI-less ticks since the last autonomous change. This expected no-op run will finish successfully."
  exit 0
fi
