...
source_commit="${REJECTED_CANDIDATE_SOURCE_COMMIT:-}"
...
if [[ -n "$source_commit" ]]; then
  if ! [[ "$source_commit" =~ ^[0-9a-f]{40}$ ]] || ! git cat-file -e "${source_commit}^{commit}" 2>/dev/null; then
    echo "REJECTED_CANDIDATE_SOURCE_COMMIT must identify a local candidate commit." >&2
    exit 2
  fi
  ...
  # Worktree policy has already constrained the candidate to these paths. Build
  # a commit directly from the index so main and the working tree never move.
  git restore --staged -- .
  candidate_paths=()
