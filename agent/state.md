...
- Key Ecological Drivers:
  ...
  - Fungal-root network interactions were reviewed and verified with new test coverage for `fungal-root-symbiont`, and observability events were added to track this interaction. Robustness was further improved by restricting `mycelial-root-mediator` to only activate when a fungal network is present, ensuring the tripartite animal-fungi-root loop is correctly gated. `nutrient-refiner` was enhanced to work during starvation for animals, fostering resilience.
